...
<td>' . $r -> state . '</td>
<td>' . $r -> last_update . '</td>
<td><a class="btn btn-info"  href="' . base_url() . 'admin/perm">Thay đổi quyền truy cập</a></td>
<td><a class="btn btn-info"  href="' . base_url() . 'admin/role/edit/' . $r -> id . '">Sửa</a></td>
<td><a class="btn btn-danger" href="' . base_url() . 'admin/role?action=delete&id=' . $r -> id . '&order=' . $sort . '" onclick="return confirm(\'Ban thuc su muon xoa\')">Xóa</a></td>
</tr>';
								}
							}
							?>
