...
class Func extends CI_Controller {
	...
	function index() {
			...
			switch ($action) {
				case 'delete' :
					/*$id = intval($this -> input -> get('id'));
					 if ($id) {
					 if ($id != LOWEST_USER_ID && $id != HIGHEST_USER_ID) {
					 $this -> Func_model -> remove_func_by_id($id);
		...
		//end get params from get method
			...
			$data['func_list'] = $this -> Func_model -> get_func('*', $where, $like, ($page - 1) * $per_page, $per_page, array());
