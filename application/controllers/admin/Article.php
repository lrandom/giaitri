...
class Article extends CI_Controller {
	...
	function show_form_add_news() {
		$select = 'cls_idSubject,cls_nameSubject';
		$array_where = array('cls_level' => 1, 'cls_displayMenu' => 1);
		$array_like = array();
		$order_by = array();
		$data['subject'] = $this -> subject -> get_subject($select, $array_where, $array_like, 0, 100, $order_by);
		$this -> load -> view('admin/form/frmAddNews', $data);
	}

	function show_form_edit_news() {
		if (isset($_GET['id'])) {
			$idNews = $this -> input -> get('id');
			$data['news'] = $this -> news -> get_news_by_id_news($idNews, TRUE);
			$this -> load -> model('image');
			$data['img'] = $this -> image -> get_for_news($idNews);
			$this -> load -> model('video');
			$data['video'] = $this -> video -> get_for_news($idNews);
			$this -> load -> model('audio');
			$data['audio'] = $this -> audio -> get_for_news($idNews);
			$data['relative_news'] = $this -> news -> get_relative_news($idNews);
			$data['idNews'] = $idNews;
			$this -> load -> view('admin/form/frmEditNews', $data);
		}
	}
