...
class User extends CI_Controller {
	...
	function add() {
		if (isset($_POST['txtFullName'])) {
			...
			$this -> load -> model('User_model');
			$data_array = array('full_name' => $txtFullName, 'user_name' => $txtUserName, 'pass' => $txtPassWord, 'email' => $txtEmail, 'phone' => $txtPhone, 'address_ip' => $this -> input -> ip_address(), 'date_join' => date('Y-m-d H:i:s', time()));
			$this -> User_model -> insert_user($data_array);
			$this -> load -> model('Role_model');
			$this->Role_model->insert_users_roles($role_id);
			$data['alert_state'] = ALERT_STATE_SUCCESS;
