...
class Article extends CI_Controller {
	...
	function index() {
		...
		if ($this -> input -> get('action')) {
			...
			switch ($action) {
				...
				if ($id) {
					$data=$this->Article_model-> get_article_by_id($id);
					if($data!=null){
	...
	function add() {
		...
		//insert data
		if (isset($_POST['txtTitle']) && isset($_POST['token'])){
			if(isset($_SESSION['token']) && ($_SESSION['token']==$this->input->post('token'))){
				...
				$date_post=date('Y-m-d H:i:s',strtotime($this->input->post('txtDatePost')));
				$this -> load -> model('Article_model');
				$data_array = array(
					'title' => $title,
					'thumb' => $thumb, 
					'cat_id' => $list_of_cat_id,
					'source' => $source,
					'author' => $author, 
					'content' => htmlspecialchars($content),
					'intro' => $intro, 'state' => DISABLED_STATE, 
					'date_post' =>$date_post , 'user_id' => 2, 
					'rel_article_id' => $list_of_rel_id);
